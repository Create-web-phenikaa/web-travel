...
function clickBlog() {
  document.addEventListener('click', (e) => {
    ...
    const dataClick = data.filter((x) => Number.parseInt(x.id) === Number(target.id))
    handleFilterChange('blog', dataClick[0].id)

    const ulElement = document.getElementById('ulList')

    ulElement.textContent = ''
    const liElement = renderData(dataClick)
  ...
  const todoList = [
    {
      ...
      url: './assets/img/blog-3.avif',
      url_1: './assets/img/blog-2.avif',
      url_2: './assets/img/blog-1.avif',
    ...
    },
    {
      ...
      desc_1:
        'The ashram has separate facilities for male & female yogis and yoga learners. The morning time is set aside for males and the evening time is set aside for the females interested in evening yoga classes. You can attend the Yoga classes on all days other than Sundays. ',
      desc_2:
        'You can also get free food and accommodation at the ashram. But this facility is only available for seekers who are seriously looking for spiritual meaning in their lives. If you are one of the serious spiritual seekers then you must apply already 30 days in advance. ',
    },
  ]
